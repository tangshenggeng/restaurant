...
<div style="display: none;" id="changeNewFoodInfoModal">
		<div class="row" style="width: 700px">
			<form class="layui-form " style="margin-top: 5px" id="changeNewFoodForm"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
			  <input type="hidden" id="newFoodId" name="newFoodId"/>
			  <div class="layui-form-item">
			    <label class="layui-form-label">菜名</label>
			    <div class="layui-input-block">
		...
		layui.use(['table','form','layer'], function(){
			  ...
			  table.render({
			    ...
			    ,cols: [[ //表头
			      ...
			      ,{field: 'isShow', title: '是否展示',align:"center",style:"color:#00838F",templet: function(d){
			  ...
			  table.on('tool(newFoodListTBFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
				  ...
				  if(layEvent === 'show'){ //解决
					  var is_show = data.isShow;
				  	  var id = data.newFoodId;	
				  	  if(is_show == 100){
				  		  layer.msg("已经在展示列表了",{icon:5})
				  		  return;
				  	  }
					  $.ajax({
						  url:"${APP_PATH}/newFood/changeNewFoodShow?id="+id,
						  method:"get",
...
<button type="button" class="btn btn-danger btn-xs btn-outline" lay-event="show">展示</button>
